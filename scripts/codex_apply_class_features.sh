...
def character_exists(conn: sqlite3.Connection, character_id: int) -> bool:
    row = conn.execute("select 1 from characters where id=? limit 1;", (character_id,)).fetchone()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Apply class features up to a level onto a character.")
    ...
    ap.add_argument("--level", type=int, required=True, help="Character class level to apply up to")
    args = ap.parse_args()
    ...
    with connect(p) as conn:
        ...
        conn.commit()
        print(f"Applied class features: class={args.class_name} up to level={args.level}")
        print(f"Feature rows considered: {len(feature_ids)} | added: {added} | already present: {skipped}")

        # Show the names of the features now on the character (top 50)
        rows = conn.execute(
            """
...
echo "==> Added tools/apply_class_features.py"
echo "Run:"
