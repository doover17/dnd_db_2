...
Deliverables:
- Add ChoiceGroup and ChoiceOption tables
- Implement loader from raw JSON (idempotent)
- Add CLI: `load-choices`, `verify-choices`
- Add verification checks for choices
- Add tests covering: create, rerun no duplicates, update adds one option

Scope:
- ONLY fighting style and generic choose-N-from-list
- No spell choices, no expertise, no invocations

Definition of Done:
- `python -m dnd_db.cli load-choices` works
- rerun idempotent
- `python -m dnd_db.cli verify-choices` passes
- `pytest` passes
...
Deliverables:
- Extend loader patterns to detect spell choice sets + expertise + invocations
- Update choice_type taxonomy accordingly
- Tests per new choice type
- Verification updates

Scope:
- Still declarative only (no character application)
